...
All work must reflect Alterspective's core values. See `C:\GitHub\Alterspective-Intelligence\Principles\Web\standards\COMPANY-VALUES-STANDARDS.md` for full details.
...
See `C:\GitHub\Alterspective-Intelligence\Principles\Web\standards\ETHICS-STANDARDS.md` for the full ethics framework.
...
| KB | Location | Purpose |
|----|----------|---------|
| Alterspective Intelligence | `C:\GitHub\Alterspective-Intelligence` | Standards, skills, patterns, and reference docs (primary) |
...
5. **CURRENT_STATE.md** — Update `docs/CURRENT_STATE.md` when the actual system state changes (create if it doesn't exist).
...
cp .env.example .env
# Fill in VITE_AZURE_CLIENT_ID, VITE_AZURE_TENANT_ID, VITE_REDIRECT_URI=http://localhost:3000

# 3. Start both client and server in watch mode
npm run dev
```

**Client** runs at `http://localhost:3000` (Vite HMR — see `src/client/vite.config.ts`).
**Server** runs at `http://localhost:3001` (tsx watch).
Vite proxies `/api/*` → `http://localhost:3001` in dev mode (configured in `src/client/vite.config.ts`).
...
| `VITE_REDIRECT_URI` | `.env` (dev) / Coolify build arg (prod) | `http://localhost:3000` (dev) |
...
| `CORS_ORIGIN` | `.env` (dev) / Coolify env (prod) | `http://localhost:3000` (dev) |
...
**Generated By**: aio init + manual review passes
